...
func LoadX509KeyPair(host string) (cert *x509.Certificate, key any, err error) {
	//err = exec.Command("build/certs/gen_cert.sh", host, strconv.Itoa(mathRand.Int())).Run()
	//if err != nil {
	//	log.Fatal(err)
	//}

	cf, err := ioutil.ReadFile("build/certs/hck.crt")
	if err != nil {
		return nil, nil, err
	}

	kf, err := ioutil.ReadFile("build/certs/cert.key")
	if err != nil {
		return nil, nil, err
	}
